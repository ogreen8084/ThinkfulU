'''
Gave variables more descriptive names & fixed line too long.
...
'''
iterate through education DataFrame, by country. If there is a 
match between countries in both DataFrames add a list of the 
years of education for men, women along with the applicable year 
...
for item in education_df.index:
	if item in gdp_df.index:
		get_info[item]=list(
			[education_df.Year[item],education_df.Men[item], 
			education_df.Women[item],
...
women_list = []
men_list = []
for loc, ed_info in get_info.iteritems():
	if ed_info[3] != '':
		men_list.append(ed_info[1])
		women_list.append(ed_info[2])
		gdp_list.append(np.log(ed_info[3]))
	else:
		pass

#convert lists of information to numpy arrays to run regression
x_var = np.array(gdp_list)
women1 = np.array(women_list)
men1 = np.array(men_list)

#fit data for women to regression line
fitwmn = np.polyfit(x_var,women1 , 1)
fit_wmn=np.poly1d(fitwmn)

#fit data for ment to regression line
fitmen = np.polyfit(x_var,men1, 1)
fit_men = np.poly1d(fitmen)

#plot women data
plt.figure(1)
plt.plot(x_var, women1, 'yo', x_var, fit_wmn(x_var), '--k')
plt.xlabel('LOG-GDP WOMEN')
plt.ylabel('EDUCATION IN YEARS')

#plot men data
plt.figure(2)
plt.plot(x_var, men1, 'yo', x_var, fit_men(x_var), '--k')
